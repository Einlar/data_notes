...
\lesson{31}{20/05/20}
%History of population dynamics part 2 %TODO Rewrite this part
...
%Plot of the occupation of the 20 most abundand species in the Barro Colorado Island (BCI) tropical forest 
...
\end{align*}
where $K$ is the normalization constant, and $\mathrm{M}$ acts as a correlation matrix. %Here we treat n as a continuous variable
...
%Plot of interaction matrix %slide 35
...
%Plot of scrambled matrix

The resulting interactions are much higher. So in nature, we see that species that coexist \textit{do not interact much}. 

Why are they not interacting in a cooperative way? Recall that in SM the MaxEnt is obtained when the interaction is \q{turned off}. The interaction potential is a constraint - it bounds the possible values of entropy, in a sense it creates \textit{order}. In nature, the system seems to like \q{being the most random possible}, and this is only possible if the interactions are very small.
...
% \begin{figure}[H]
%     \centering
%     \includegraphics[width=0.8\textwidth]{preston-fits.png}
