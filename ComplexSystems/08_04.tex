...
%1. This means that an ensemble occupying a phase-space volume $A_0$ cannot evolve to a \textit{smaller} volume, but nothing prevents the microstates associated with the final macrostate reached from $A_0$ to occupy a larger volume than $A_0$. 

%2. meaning that at \textit{equilibrium}, the relative weights of different regions of phase-space depend only on their volumes, as there cannot be a smaller patch with \q{higher} density of states. Initial uniform density will stay uniform
